...
from utils import prepare_mask_and_masked_image_for_pipeline
...
class StableDiffusionInpaintPipeline(
    ...
    def __call__(
        ...
        mask, masked_image, init_image = prepare_mask_and_masked_image_for_pipeline(
        ...
        image = postprocess(self.image_processor.config, image, output_type=output_type, do_denormalize=do_denormalize)
...
def denormalizeNew(images: Union[np.ndarray, torch.Tensor]) -> Union[np.ndarray, torch.Tensor]:
    """
    Denormalize an image array to [0,1].
    """
    return ((images + 1) * 0.5).clamp(0, 1)    
...
def postprocess(
        ...
        if do_denormalize is None:
            do_denormalize = [config.do_normalize] * image.shape[0]    #################################################################

        image = torch.stack(
            [denormalizeNew(image[i]) if do_denormalize[i] else image[i] for i in range(image.shape[0])]    ##############""
        )
